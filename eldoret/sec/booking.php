      ...
      <?php
        ...
        $sql = "SELECT * FROM vehicle_seats WHERE  number_plate= '$plate_no' AND pri_key='$pri_key'";
          ...
          if (mysqli_num_rows($result) > 0) {
            ...
            <button class="btn btn-success text-success bg-transparent fa fa-check" style="width:50%" name='submit'>Submit</button>
            <button class="btn btn-danger bg-transparent text-danger fa fa-times" onclick="window.history.back();" style="width:40%">Cancel</button>
            </form>
...
if (isset($_POST['submit'])){ 
    ...
    if($conn->query($sql) === TRUE) {
        echo "<script>
            window.location='confirm.php?pri_key=$pri_key&fname=$fname&lname=$lname&date=$date&departure=$departure&start=$start&ticket_id=$ticket_id&end=$end&phone=$phone&price=$price&seat_no=$seat_no&plate_no=$plate_no';
        </script>";
    } else {
