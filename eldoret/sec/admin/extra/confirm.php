...
<div class="container">
  ...
  <div class="col-md-4">
    ...
    <div class="col-md-4 p-3">
            <fieldset style="border: 1px white solid; width: 100%; box-shadow: 2px 4px 4px 0px red;" class="p-4 text-light border-light text-center">
                <legend class="text-light text-info h5" style="text-shadow: 0px 1px 1px red;">Booking Summary</legend>
      <form method="POST" action=" ">                  
      <div class="card text-light bg-transparent border-drak">
        <div class="card-body text-left text-success" style="font-size: 12px;"><input type="text-light" name="pri_key" value="<?php echo $pri_key; ?>" hidden>
          ...
          Phone: <span class="text-light">0<?php echo $phone; ?></span><br>
        </div>
      </div>
      <div class="card pt-3 bg-transparent text-light">
        <div class="h6 bg-dark text-success p-1 text-center">Payment Method</div>
        <span class="text-warning text-left pl-2" style="font-family: serif; font-size: 12px;"><i>Please select the payment method to complete your booking</i></span><br>
        <span class="text-light text-left pl-2"><input type="radio" name="pay_method" required value="cash"> <i class="fa fa-money robot">Cash</i> </span>
        <span class="text-light text-left pl-2"><input type="radio" name="pay_method" required value="mpesa"/> <i class="fa fa-telegram robot"> Mpesa</i></span><br>
        <div class="row">
            <div class="col-12">
    ...
    if($conn->query($sql) === TRUE) {
        ...
        $ql = "UPDATE vehicle_avail SET booked_seat= booked_seat + 1 WHERE pri_key='$pri_key'";
            if ($conn->query($ql) === TRUE) {
              echo "<script>alert('Seat Reseaved successfully');</script>";
            } else {
